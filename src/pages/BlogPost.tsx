...
import { CONTENT_BASE_URL, SITE_BASE_URL, RAW_REPO_URL } from "../config";
    ...
    const fetchPost = async () => {
      try {
        ...
        if (processedPostMeta.type === "pdf") {
          ...
          window.open(
            `${SITE_BASE_URL}/${processedPostMeta.content}`,
            "_blank",
            "noopener noreferrer",
          );
          ...
          const contentResponse = await fetch(
            `${RAW_REPO_URL}/${processedPostMeta.content}`,
          );
  ...
  if (loading) {
    return (
      <div className="flex justify-center items-center min-h-[50vh]">
        <p className="text-lg">{t("blogPost.loading")}</p>
      </div>
    );
  }
    ...
    return (
      <div className="flex flex-col justify-center items-center min-h-[50vh] space-y-4">
        <p className="text-lg text-red-500">
          {t("blogPost.errorPrefix")}{error || t("blogPost.notFound")}
        </p>
        <a
          href="/blog"
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between">
            <time className="text-sm block" dateTime={post.date}>
              {(() => {
                try {
                  return new Date(post.date).toLocaleDateString(i18n.language === "pt" ? "pt-BR" : "en-US", {
                    year: "numeric",
                    month: "long",
                    day: "numeric",
                  });
