...
export default function ReadingList() {
  ...
  useEffect(() => {
    fetch(
      `${CONTENT_BASE_URL}/reading_list.json`,
    )
      .then((res) => res.json())
      .then((data) => {
        try {
      ...
      .catch((error) => {
        ...
        setError(t("readingList.fetchError"));
        setLoading(false);
      });
  }, []);
  ...
  return (
    ...
    >
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <h1 className="text-4xl font-bold">{t("readingList.title")}</h1>
              ...
              <div className="flex flex-col gap-3">
                <div className="flex flex-col sm:flex-row sm:justify-between sm:items-start gap-2">
                  ...
                  <div className="text-sm text-[color-mix(in_oklch,var(--color-primary)_60%,transparent)] shrink-0">
                    {t("readingList.addedLabel")}{new Date(paper.dateAdded).toLocaleDateString(i18n.language, {
                      year: "numeric",
                      month: "long",
                      day: "numeric",
                    })}
